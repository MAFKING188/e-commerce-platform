...
<div class="partner-profile-header">
    ...
    <div class="partner-meta">
        ...
        <h1>{{ $partner->name }}</h1>
        <p>{{ $partner->description ?? 'No description provided for this luxury partner.' }}</p>
        
        <div style="display: flex; gap: 2rem; align-items: center;">
            @if($partner->website)
                <a href="{{ $partner->website }}" target="_blank" class="btn btn-ghost">Visit Website</a>
            @endif
            <div style="color: var(--text-400); font-size: 0.9rem;">
                <strong>Contact:</strong> {{ $partner->contact_info }}
            </div>
        </div>
    </div>
</div>
...
<div class="inventory-section">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 3rem;">
        ...
        <form action="{{ route('admin.partners.add_product', $partner->id) }}" method="POST" style="display: flex; gap: 1rem; align-items: center; background: var(--surface-100); padding: 0.5rem 1rem; border-radius: 1.5rem; border: 1px solid var(--border);">
            @csrf
        ...
        @foreach($partner->products as $product)
            <div class="mapping-card">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1.5rem;">
                    ...
                    <form action="{{ route('admin.partners.remove_product', [$partner->id, $product->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
