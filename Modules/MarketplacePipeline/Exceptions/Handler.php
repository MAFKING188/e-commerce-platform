...
namespace Modules\MarketplacePipeline\Exceptions;
...
class Handler extends FoundationHandler
{
    public function report(Throwable $e): void
    {
        ...
        if ($role !== 'admin') {
            $this->channel($request, $e, 'pipeline-errors');
        } else {
            parent::report($e);
        }
    }
    ...
    public function render($request, Throwable $e): Response|string
    {
        if ($e instanceof ValidationException) {
            return response()->view('marketplacepipeline::errors.validation', [], 422);
        }

        if ($e instanceof AuthorizationException) {
            return response()->view('marketplacepipeline::errors.404', [], 403);
        }

        $code = $e->getCode() ?: 500;

        return response()->view(
            'marketplacepipeline::errors.' . $code,
            [],
            $code
        );
    }
