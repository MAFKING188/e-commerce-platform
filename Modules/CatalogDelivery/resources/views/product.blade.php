...
<div class="product-details">
    ...
    <div class="product-info">
        ...
        <button type="button" class="btn-share" onclick="shareProduct()" aria-label="Share this product" style="display: inline-flex; align-items: center; gap: 0.5rem; margin-top: 0.75rem; padding: 0.5rem 1rem; border: 1px solid var(--border); border-radius: 8px; background: var(--surface-100); color: var(--text-600); font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: all 0.2s;">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
            Share
        </button>
        <span id="share-toast" style="display: none; margin-left: 0.5rem; font-size: 0.8rem; color: #16a34a; font-weight: 600;">Link copied!</span>

        <script>
        function shareProduct() {
            const url = '{{ url()->current() }}';
            const title = '{{ addslashes($product->name) }} — SmartShop';
            const text = 'Check out {{ addslashes($product->name) }} on SmartShop — ${{ number_format($product->price, 2) }}';
            if (navigator.share) {
