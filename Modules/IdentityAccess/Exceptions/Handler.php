...
namespace Modules\IdentityAccess\Exceptions;
...
class Handler extends FoundationHandler
{
    public function report(Throwable $e): void
    {
        ...
        if ($role !== 'admin') {
            $this->channel($request, $e, 'identity-errors');
        } else {
            parent::report($e);
        }
    }
    ...
    {
        if ($e instanceof ValidationException && $e->getValidator()) {
            return view('identity::errors.validation');
        }

        if ($e instanceof ValidationException) {
            // ValidationException without validator - return view directly
            return view('identity::errors.validation');
        }

        if ($e instanceof AuthorizationException) {
            return view('identity::errors.404');
        }

        $code = $e->getCode() ?? 500;

        return view('identity::errors.' . $code);
    }
}